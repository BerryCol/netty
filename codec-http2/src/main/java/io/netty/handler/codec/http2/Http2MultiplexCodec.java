...
public class Http2MultiplexCodec extends Http2FrameCodec {
    ...
    final void onHttp2StreamWritabilityChanged(ChannelHandlerContext ctx, Http2FrameStream stream, boolean writable) {
        ...
        channel.writabilityChanged(writable);
    }
    ...
    private final class Http2MultiplexCodecStreamChannel extends AbstractHttp2StreamChannel {
        ...
        Http2MultiplexCodecStreamChannel(DefaultHttp2FrameStream stream, ChannelHandler inboundHandler) {
            super(stream, ++idCount, Http2MultiplexCodec.this.initialWritability(stream), inboundHandler);
        }

        @Override
        protected int initialOutboundStreamWindow() {
            return Http2MultiplexCodec.this.initialOutboundStreamWindow;
        }

        @Override
        protected boolean isWritable(DefaultHttp2FrameStream stream) {
            return Http2MultiplexCodec.this.isWritable(stream);
        }
