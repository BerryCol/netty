...
public final class Http2MultiplexHandler extends Http2ChannelDuplexHandler {
        ...
        if (evt instanceof Http2FrameStreamEvent) {
            ...
            switch (event.type()) {
                case State:
                    ...
                    break;
                case Writability:
                    ((AbstractHttp2StreamChannel) stream.attachment).writabilityChanged(isWritable(stream));
                    break;
                default:
                    // ignore
                    break;
    ...
    private final class Http2MultiplexHandlerStreamChannel extends AbstractHttp2StreamChannel {
        ...
        Http2MultiplexHandlerStreamChannel(DefaultHttp2FrameStream stream, ChannelHandler inboundHandler) {
            super(stream, ++idCount, Http2MultiplexHandler.this.initialWritability(stream), inboundHandler);
        }

        @Override
        protected int initialOutboundStreamWindow() {
            return initialOutboundStreamWindow;
        }

        @Override
        protected boolean isWritable(DefaultHttp2FrameStream stream) {
            return Http2MultiplexHandler.this.isWritable(stream);
        }
